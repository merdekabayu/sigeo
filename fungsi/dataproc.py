...
def fromdbsta(dt_0,dt_1):
    ...
    for dt in daterange(start_dt, end_dt):
        ...
        f_bln = str(dt.strftime('%-m'))
        filename = str(dt.strftime("%Y%m%d") + '.db3')
        dir = '../sipetir/fungsi/data/DB3/' + f_thn + '/' + f_bln
...
def readdb3(databases,fileout,batas):
    ...
    f.close()
    f = open(fileout, 'r')
    fcgp = open('fungsi/sambarancgp.dat', 'w')
    fcgm = open('fungsi/sambarancgm.dat', 'w')
    fcg = open('fungsi/sambarancg.dat', 'w')
    fic = open('fungsi/sambaranic.dat', 'w')
    baris = f.readlines()
